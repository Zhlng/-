...
export default defineComponent({
  ...
  methods: {
    ...
    getRelation() {
      axios
        .get("http://localhost:8080/relation/list")
        .then((res: any) => {
          this.rlist = res.data || [];
          this.getData();
        });
    ...
    getMembers() {
      this.list = [];
      axios
        .post("http://localhost:8080/personnel/chat", { familyId: this.familyId })
        .then((res: any) => {
          this.list = res.data;
          for (let i = 0; i < this.list.length; i++) {
            for (let j = 0; j < this.list.length; j++) {
              ...
              for (let z = 0; z < this.rlist.length; z++) {
                if (
                  this.rlist[z].personnelId1 == this.list[i].personnelId &&
                  this.rlist[z].personnelId2 == this.list[j].personnelId
                ) {
              ...
              if (this.flag) {
                if (this.form.relationType == "夫妻") {
                  this.list[j].pid = this.form.personnelId1 + "/0";
                } else {
                  this.list[j].pid = this.list[i].personnelId;
                }
              }
            }
          }
          for (let i = 0; i < this.list.length; i++) {
            if ((this.list[i].pid+'').indexOf("/") > -1) {
              for (let j = 0; j < this.list.length; j++) {
                if (this.list[i].pid == (this.list[j].personnelId + '/0')) {
                  this.list[i].pid = this.list[j].pid
                  this.list[i].personnelName += "("+this.list[j].personnelName+"妻子)" 
                }
              }
            }
          }
    ...
    getTreeList(list: Array<Object>, id: string, tree: any) {
      ...
      tree.forEach((item) => {
        item.children = [];
        this.getTreeList(list, item.personnelId, item.children);
      });
      return tree;
    },
    drawTree() {
